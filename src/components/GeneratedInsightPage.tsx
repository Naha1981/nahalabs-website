    ...
    <div className="min-h-screen bg-[#080909] text-[#F3F0EA]">
      <header className="sticky top-0 z-40 border-b border-[#1b1c1f] bg-[#080909]/92 backdrop-blur-md">
        <div className="max-w-5xl mx-auto px-5 sm:px-8 py-4 flex items-center justify-between">
          <a href="/" className="text-[11px] font-mono uppercase tracking-[0.18em] text-[#A5A29B] hover:text-[#F3F0EA]">
            NahaLabs
          </a>
          <a href="/#contact" className="text-[10px] font-mono uppercase tracking-wider font-semibold border border-[#2b2d35] rounded-full px-4 py-2">
      ...
      <main className="max-w-5xl mx-auto px-5 sm:px-8 py-16 sm:py-24">
        <div className="max-w-3xl">
          ...
          <h1 className="text-4xl sm:text-6xl leading-[1.02] font-semibold tracking-[-0.035em]">
            {bundle.title}
          </h1>
          ...
          <div className="mt-8 flex flex-wrap gap-3 text-[10px] font-mono uppercase tracking-[0.16em] text-[#666]">
            <span>{bundle.datePublished}</span>
            <span>·</span>
        ...
        <div className="mt-12 max-w-3xl space-y-9 text-[17px] sm:text-[18px] leading-[1.85] text-[#D7D3CA]">
          {bundle.blocks.map((block, index) => {
            if (block.type === "h2") {
              return (
                <h2 key={index} className="text-2xl sm:text-3xl font-semibold text-[#F3F0EA] pt-6">
                  {block.text}
                </h2>
              );
            }
            ...
            if (block.type === "quote") {
              return (
                <blockquote key={index} className="border-l-2 border-[#C8AE82] pl-6 text-[#F3F0EA] font-medium">
                  {block.text}
                </blockquote>
              );
            }
              ...
              <div className="mt-5 space-y-4">
                {bundle.sources.map((source) => (
                  <div key={source.url} className="border-b border-[#24262a] pb-4">
                    <a href={source.url} target="_blank" rel="noreferrer" className="text-[#C8AE82] underline underline-offset-4">
                      {source.title}
                    </a>
